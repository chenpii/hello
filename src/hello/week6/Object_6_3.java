...
public class Object_6_3 {
	/*字符串操作
	 * 字符串是对象，对它的所有操作都是通过. 进行的
	 * 	字符串.操作
	 * 它表示对.左边的这个字符串做右边的操作
	 * 这里的字符串可以是变量，也可以是常量
	 * 
	 * 1.String大小的比较(Unicode) comparteTo()
	 * 两个字符串可以比较大小 s1.comparteTo(s2)
	 * s1<s2小，结果是负的
	 * s1=s2，结果是0
	 * s1>s2，结果是正的
	 * compareToIgnoreCase 不区分大小写来比较大小
	 * 
	 * 2.获得String的长度 length()
	 * s1="hello"  	s1.length()=5
	 * s1=""  		s1.length()=0
	 ...
	 * 			s.lastIndexOf(t)
	 * 6.其他操作
	 * 		s.startsWith(t) 	是否以某字符串开头
	 * 		s.endsWith(t)		是否以某字符串结尾
	 * 		s.trim()			将字符串两端空格去掉
	 * 		s.replace(c1,c2)	将c1换成c2
	 * 		s.toLowerCase()		把字母都换成小写
	 * 		s.toUpperCase()		把字母都换成大写
	 * 
	 * 注意：所有的操作不会改变字符串本身，都是新建一个字符串将操作的结果存起来
	 * 
	 * 7.可以在switch-case中使用字符串
	 * switch(s){
	 * case"this":...break;
	 * case"that":...break;
	 * }
